...
export const createVocabController = asyncHandler(async (req: Request, res: Response) => {
  sendCreated(
    res,
    await createVocabService(req.body as CreateVocabInput, req.user!.userId),
    'Kartu vocab berhasil dibuat',
  );
});
...
export const submitReviewController = asyncHandler(async (req: Request, res: Response) => {
  sendSuccess(
    res,
    await submitReviewService(req.body as ReviewInput, req.user!.userId),
    200,
