...
## Feature: Obrolan Grup per Materi

### PesanObrolan
```typescript
interface PesanObrolan extends BaseEntity {
  materiId: string;
  userId: string;
  userNama: string;
  userRole: "GURU" | "SISWA";
...
| Method | Path | Keterangan |
|---|---|---|
| `GET` | `/obrolan?materiId=` | Semua pesan dalam materi (auth required) |
| `POST` | `/obrolan` | Kirim pesan; body: `{ materiId, teks }` — backend populate userId/userNama/userRole dari session |
