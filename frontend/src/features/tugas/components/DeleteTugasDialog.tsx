...
import { deleteTugasAction } from "../actions/delete-tugas.action";

export function DeleteTugasDialog() {
  const { deleteTargetId, deleteTargetName, closeDeleteDialog } = useTugasUIStore();
  ...
  function handleDelete() {
    startTransition(async () => {
      await handleAction(await deleteTugasAction(deleteTargetId!), { successMsg: "Tugas berhasil dihapus.", onSuccess: closeDeleteDialog });
    });
  }
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
        <div className="flex items-start gap-4 mb-6">
          ...
          <div>
            <h2 className="text-base font-semibold text-slate-900 mb-1">Hapus Tugas</h2>
            <p className="text-sm text-slate-600">Tugas <span className="font-medium text-slate-900">{deleteTargetName}</span> dan semua submisi siswa akan dihapus permanen.</p>
          </div>
        </div>
